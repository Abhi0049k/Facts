...
import type { AnalyzeResponse, PipelineState } from "@/lib/types";
...
export async function runPipeline(
  ...
  state.userCompany = await understandCompany(rawContent, input.companyUrl);
  ...
  await emitStage(onEvent, 3, "complete");
  ...
  await emitStage(onEvent, 4, "complete");
  ...
  await emitStage(onEvent, 5, "complete");
  ...
  if (input.includeSentiment) {
    ...
    await emitStage(onEvent, 8, "complete");
  }
